...
/**
  ...
  * 说    明：使用自定义的27x38字模显示
  */
void OLED_ShowBigNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length)
{
	uint8_t i;
	// 遍历数字的每一位
	for (i = 0; i < Length; i++) 
	{
		// 1. 提取每一位数字
		// 2. 调用OLED_ShowImage显示该数字对应的27x38图像
		// 3. 数字宽度为27，所以X坐标每次增加27
		uint8_t singleNum = Number / OLED_Pow(10, Length - i - 1) % 10;
		OLED_ShowImage(X + i * 27, Y, 27, 38, OLED_F27x38[singleNum]);
	}
}
