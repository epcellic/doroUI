...
int main(void)
{
    ...
    OLED_ShowString(45, 50, "Ver.1.2", OLED_6X8);
    OLED_Update();
    Delay_ms(500);
    OLED_Clear();OLED_Update();
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else	// 处理当前二级页面的自定义按键（如LED控制、参数调节等）
            {
...
void OLED_DrawSubPage(PageType page)
{
    ...
    switch (page)
    {
        case PAGE_AD:
            ...
            OLED_ShowString(80, 40, "0-4096", OLED_6X8); 
            OLED_ShowString(1, 55, "enter:key1", OLED_6X8);
            break;
        
        case PAGE_DWAVE:
            OLED_ShowString(10, 15, "数字波形页面", OLED_8X16);
            OLED_DrawLine(10, 30, 118, 30); // 绘制波形基线
            OLED_ShowString(1, 55, "swtich:key1", OLED_6X8);
            break;
        
        case PAGE_PICTURE:
            OLED_ShowString(30, 15, "图片页面", OLED_8X16);
            OLED_ShowImage(40, 30, 38, 38, doro3); // 需替换为你的图片数组
            break;
        
        case PAGE_LED:
            OLED_ShowString(30, 15, "LED控制", OLED_8X16);
            OLED_ShowString(10, 35, "LED1: 关闭", OLED_8X16); // 占位，后续可接GPIO状态
            OLED_ShowString(1, 55, "swtich:key1", OLED_6X8);
            break;
        
        case PAGE_GAME:
            OLED_ShowString(30, 15, "小游戏", OLED_8X16);
            OLED_ShowString(10, 35, "方向:按键3/4", OLED_8X16);
            OLED_ShowString(1, 55, "enter:key1", OLED_6X8);
            break;
        
        case PAGE_USART:
            OLED_ShowString(10, 15, "串口打印页面", OLED_8X16);
            OLED_ShowString(10, 35, "发送:welcome", OLED_8X16);
            OLED_ShowString(1, 55, "send:key1", OLED_6X8);
            break;
        
        default:
            OLED_ShowString(30, 30, "页面错误", OLED_8X16);
            break;
    }
}

// 处理二级页面的按键输入（每个页面独立逻辑）
void OLED_HandleSubPageInput(PageType page)
{
    if (KeyNum != 0) // 有按键按下时才处理
    {
        switch (page)
        {
            ...
            // 其他二级页面的按键逻辑（如波形切换、游戏控制）可在此扩展
            case PAGE_DWAVE:
            case PAGE_PICTURE:
            case PAGE_GAME:
            case PAGE_USART:
                // 按需添加逻辑（例如游戏方向控制、串口发送等）
                break;
            default:
                break;
        }
