...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说明：配置 PC6为下拉输入(高电平触发)，PC7~PC9 为上拉输入(低电平触发)，对应4个按键
  */
void Key_Init(void)
{
	/*开启时钟*/
	...
	/*先初始化PC7~PC9为上拉输入（低电平触发）*/
	GPIO_InitTypeDef GPIO_InitStructure;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;				//上拉输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9;	//C7~C9三引脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;				//GPIO速度
	GPIO_Init(GPIOC, &GPIO_InitStructure);						//初始化GPIOC
	
	/*单独初始化PC6为下拉输入（高电平触发）*/
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD;				//下拉输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;					//仅C6引脚
	GPIO_Init(GPIOC, &GPIO_InitStructure);						//初始化GPIOC
}
...
/**
  ...
  *           1-PC6按键(高电平触发)，2-PC7按键(低电平触发)，3-PC8按键，4-PC9按键
...
uint8_t Key_GetNum(void)
{
	...
	{
		Delay_ms(20);											//延时消抖
		while (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_6) == 1);	//等待按键松手
		Delay_ms(20);											//延时消抖
		KeyNum = 1;												//置键码为1
	}
